...
/* 
 This software is provided 'as-is', without any express or implied
  ...
  2. Altered source versions must be plainly marked as DYNAMICsuch, and must not be
...
*/
	...
	public:
			...
			Quaternion& operator*=(const Quaternion& other);			// Multiplication operator
			...
			// Inverse operators 
			inline Quaternion& makeInverse() { X = -X; Y = -Y; Z = -Z; return *this; }	// invert this Quaternion			
			inline Quaternion operator~() { return Quaternion(-X, -Y, -Z, W); }  // return inverse, W unchanged
			inline Quaternion inverse ()  {	return Quaternion(-X, -Y, -Z, W); }  // return inverse, W unchanged			
			Quaternion conjInverse ();									// return conjugate inverse
			Quaternion negative() { return Quaternion(X, Y, Z, -W); }	// return negative quaternion (-W)
			Quaternion& dual(Vector3DF pos);							// return dual quaternion
